...
\paragraph{Phase 2: Layer freezing (optional).} The first $K$ transformer layers may be frozen during specialist training. Frozen layers guarantee that lower-level representations remain shared across specialists---a structural anchor that prevents over-specialisation under extended training. Freezing is unnecessary below approximately 10,000 specialist training steps but becomes beneficial beyond that threshold (Section~\ref{sec:crossover}).
...
\paragraph{Divergence floor: derivation and scope.}
The predictive formula (Section~\ref{sec:divergence_gain}) predicts zero gain at $\text{divergence} = 2.80 / 0.83 \approx 3.4\%$. This floor is \emph{regime-scoped}: it applies across fixed training protocols differing in model scale or domain (the eight in-sample conditions in Table~\ref{tab:divergence}), where divergence and gain are measured at the same training step count (2,000 steps) per condition. Within a step-count sweep, the relationship is different: at 50 specialist training steps, mean specialist divergence is only 2.04\%---below the 3.3\% floor---yet fusion gain vs.\ best specialist is +5.06\% (Table~\ref{tab:crossover}). This apparent contradiction resolves cleanly: at 50 steps, the ``best specialist'' is itself barely different from the base model (it has trained for 50 steps on one domain), so the MoE, combining three near-base specialists with a trained router, outperforms the single best near-base specialist without needing large divergence. As training steps increase, divergence grows, both the specialists and the fusion benefit grow, and the step-sweep tracks the regime that the cross-condition regression was fit on. The practical implication: (i) the 3.3\% floor is a valid pre-training diagnostic for deciding whether a cooperative is worth starting---if expected specialist divergence after your planned training budget is below 3.3\%, expect near-zero gain; (ii) within an ongoing cooperative, even very short training contributions are beneficial.
...
The same privacy property that makes the protocol beneficial introduces a dual-use risk: contributors can train specialists on data that is entirely invisible to the coordinator, which could allow harmful or biased knowledge to be incorporated into the fused model without centralised oversight. The $N{\times}$ inference overhead provides practical friction against casual misuse at scale, but does not eliminate the concern for motivated actors. Production deployments of the protocol should include contributor vetting and checkpoint validation as part of their governance design.
