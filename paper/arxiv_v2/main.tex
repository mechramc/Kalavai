...
$\text{gain} \approx 0.83 \times \text{divergence} - 2.80$ ($R^2 = 0.872$, $n=8$, spanning 3--26\% divergence).
...
Phase 2 extends the protocol to high-divergence settings: cross-lingual fusion (Tamil/Yoruba/Welsh/Code) achieves $+21.87\%$ $\pm$0.12pp (3-seed mean; curriculum warm-start, Section~\ref{sec:phase2_crosslingual}), with Yoruba perplexity falling $41.9\to7.7$.
A 20-contributor federation (10 languages + 10 domains) achieves $+16.71\%$ ($\pm$0.07pp, 3 seeds) vs.\ best specialist; after specialist quality filtering (removing two undertrained, negative-divergence specialists), the 18-contributor ablation achieves $+21.13\%$ ($\pm$0.01pp, 3 seeds, mean divergence 19.75\%).
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  \item \textbf{Predictive divergence--gain relationship.} Fusion gain scales linearly with specialist divergence: $\text{gain} = -2.80 + 0.83 \times \text{divergence}$ ($R^2 = 0.872$, slope 95\% CI [0.52, 1.15]), validated across eight conditions from Qwen-1.5B (3.16\% divergence, +1.06\% gain) to cross-lingual (25.65\%, +21.87\%). Linear fit is substantially better than log-linear ($R^2 = 0.683$). The cross-lingual condition exceeds the linear prediction by +3.7pp, consistent with base-model incompetence creating outsized gains. Practitioners can estimate cooperative value from specialist divergence alone before committing to training.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
  \item \textbf{A predictive gain model.} Fusion improvement scales linearly with specialist divergence ($\text{gain} = 0.83 \times \text{divergence} - 2.80$, $R^2 = 0.872$, eight conditions, 3--26\% divergence). Practitioners can estimate cooperative value from divergence alone, before committing training resources.
  ...
  \item \textbf{High-divergence validation.} Phase 2 extends the protocol to private professional domains ($+10.17\%$, medical/legal/patent) and cross-lingual settings ($+21.87\%$ $\pm$0.12pp, 3-seed mean, curriculum warm-start; Tamil/Yoruba/Welsh/Code; Yoruba perplexity $41.9\to7.7$). Gain scales most steeply where the base model is least competent.
...
\paragraph{Curriculum warm-start fixes router initialisation sensitivity.} In an earlier training run (without curriculum), seed 42 collapsed Yoruba inputs onto the Tamil specialist (99.84\% gate weight): both Tamil and Yoruba are tokenizer-OOD byte-fallback scripts, and at that random seed their hidden-state representations were insufficiently differentiated for the router to separate them under standard initialisation. Curriculum warm-start (Stage A: 100 domain-pure steps, one batch per language; Stage B: 400 mixed steps to 500 total) resolves this: seed 42 achieves +22.04\%, Yoruba PPL 41.9$\to$7.7, matching seeds 137 and 2026. This finding suggests a simple mitigation for tokenizer-OOD routing sensitivity at negligible additional cost.

3-seed mean: $+21.87\%$ $\pm$0.12pp; mean divergence 25.65\%; final verdict \textbf{GO} (all seeds).
...
We validate this directly: retraining the router on the 18 quality-passing specialists (omitting dialogue and instructions) achieves $\mathbf{+21.13\%}$ $\pm$0.01pp vs.\ best specialist (3-seed mean, mean divergence 19.75\%), compared to the regression prediction of $+13.45\%$ at that divergence level (+7.68pp residual).
The mean divergence rises from 15.68\% (with toxic specialists) to 19.75\% (18 specialists only), consistent with the negative-divergence domains having suppressed the cooperative-level mean.
This confirms that the protocol requires minimally sufficient specialist training data: the divergence-gain framework predicts near-zero or negative gain when specialist divergence does not exceed the base-model competence floor ($\approx$3.3\%).

\paragraph{18-Contributor Ablation (FE-03).}
Table~\ref{tab:eighteen} reports per-seed metrics for the 18-specialist router.
The $\pm$0.01pp seed-to-seed variance (vs.\ $\pm$0.07pp for 20 specialists) reflects that the router faces a
well-conditioned problem once negative-divergence specialists are removed.
All three seeds achieve GO; STOP/GO verdict: \textbf{GO}.

\begin{table}[h]
\centering
\caption{18-contributor ablation on Pythia-1B (FE-03, 3 seeds). Same specialist checkpoints as
Table~\ref{tab:twenty} (seed 42); dialogue and instructions specialists omitted.
...
Across eight experimental conditions---Qwen-1.5B (mean div.\ 3.16\%), Pythia-6.9B (8.73\%), P1 2-domain code/science (10.77\%), Pythia-1B (15.28\%), Pythia-410M (15.65\%), Exp 2 private-domain (18.52\%), P2 4-domain code/science/fiction/legal (19.84\%), and Exp 1 cross-lingual (25.65\%)---fusion gain scales monotonically with specialist divergence (Figure~\ref{fig:divergence_gain_regression}). Exp 3 (20-contributor, 15.68\%, +16.71\%, 3-seed mean) serves as an out-of-sample validation point (Table~\ref{tab:divergence_gain}).
...
\begin{figure}[h]
  ...
  \caption{Fusion gain vs.\ mean specialist divergence (\%) with OLS regression line and 95\% prediction band. Linear fit: $\text{gain} = -2.80 + 0.83 \times \text{div}$ ($R^2 = 0.872$, $n=8$ in-sample conditions). Eight conditions spanning 3.16--25.65\% divergence; P1 2-domain (cyan) and P2 4-domain (orange) fill gaps at 10.77\% and 19.84\% divergence. English-domain conditions cluster near the line; Exp 2 (private, purple) and Exp 1 (cross-lingual, red) lie above the English-domain prediction, consistent with base-model incompetence on target domains producing outsized gains. The cross-lingual condition is the largest in-sample outlier (+3.2pp). \emph{Note:} Exp~3/FE-03 (18-contrib quality-filtered, div\,=\,19.75\%, gain\,=\,+21.13\%, 3-seed mean) is an out-of-sample validation point lying +7.46pp above the regression line (Table~\ref{tab:divergence_gain}).}
  \label{fig:divergence_gain_regression}
\end{figure}

\begin{table}[h]
\centering
\caption{Summary of divergence--gain relationship across all Phase 1 and Phase 2 experiments. Predicted gain from linear fit $= -2.80 + 0.83 \times \text{div}$; residual = actual$-$predicted. Predicted values are computed from full-precision OLS coefficients (slope $= 0.8335$, intercept $= -2.7956$); the displayed formula rounds to two decimal places, so applying it directly may differ by $\leq$0.1pp. The regression was fit on the eight in-sample conditions (rows 1--8); Exp~3 (row 9) is an out-of-sample validation point. $^{\dagger}$Qwen predicted gain ($\approx{-}0.1\%$, effectively zero) is at the divergence floor ($\approx$3.3\%); the cooperative produced modest positive gain in practice (+1.06\%). $^{\ddagger}$In a 20-specialist cooperative no single specialist achieves strong equal-weight performance across all 20 domains; the best specialist (Arabic, EW 2.7771) is only 0.46\% above base (2.7898), making gain-vs-spec $\approx$ gain-vs-base (+17.17\%). The nominal conversion rate $>1$ reflects this near-equal baseline, not an anomalous efficiency.}
...
The residual pattern is informative. The four English-domain conditions (Pythia 410M/1B/6.9B and Private) all lie within $\pm$2.6pp of the line, forming a coherent cluster. The 6.9B point sits +2.05pp above the line---consistent with larger models converting divergence more efficiently. The two regression extension points (P1 and P2) lie within +0.04pp and +0.97pp of the line respectively, confirming the linear relationship holds at intermediate divergence levels. The cross-lingual condition is the largest in-sample outlier (+3.29pp), explained by base-model near-incompetence on Yoruba and Welsh: when the base model achieves near-random perplexity on a domain, the specialist corrects this from a high baseline and the router routes with near-perfect confidence, leaving no gain on the table. The out-of-sample Exp~3/FE-03 point (+7.46pp residual, 18-specialist quality-filtered configuration, 3-seed mean) lies further above the line, consistent with its heterogeneous mix of high-divergence language specialists (Yoruba +58\%, Welsh +37\%, Swahili +38\%) pulling the cooperative gain above what the English-domain regression would predict.

The practical implication is that the formula $\text{gain} \approx 0.83 \times \text{divergence}$ provides a reliable pre-training estimate for English-domain and professional-domain cooperatives. Cross-lingual settings with low-resource languages will likely exceed this prediction. The formula also sets a divergence floor: below $\approx$3.3\% mean divergence, the linear prediction approaches zero, indicating the cooperative is unlikely to produce positive gains over individual specialists.
...
Phase 2 extends the protocol beyond English domains: private professional domains (medical/legal/patent) achieve +10.17\% gain at 18.52\% mean divergence; cross-lingual fusion (Tamil/Yoruba/Welsh/Code) achieves +21.87\% $\pm$0.12pp (3-seed mean, curriculum warm-start) at 25.65\% divergence, with Yoruba perplexity falling 5.4$\times$ (41.9$\to$7.7) and Welsh 4.6$\times$ (102.7$\to$22.1). Gain scales with divergence at a conversion rate that \emph{improves} in high-divergence settings (0.85$\times$ cross-lingual vs.\ 0.49$\times$ English domains). The protocol is most valuable precisely where it is most needed.
...
Exp 1: Cross-lingual (410M) & Pythia-410M & +21.87\% $\pm$0.12pp (curriculum, 3 seeds) & 3 GO & Done \\
Exp 3: 20-contributor (1B) & Pythia-1B & +16.71\% $\pm$0.07pp vs.\ spec (mean div.\ 15.68\%) & 3 GO & Done (seeds 42/137/2026) \\
FE-03: 18-contributor ablation (1B) & Pythia-1B & +21.13\% $\pm$0.01pp vs.\ spec (mean div.\ 19.75\%, quality filtered) & 3 GO & Done (seeds 42/137/2026) \\
...
\begin{table}[h]
\centering
\caption{Experiment 1 per-seed results. Pythia-410M step10000, freeze=0, 2,000 specialist steps. Wikipedia fallback used for Tamil/Yoruba/Welsh (cc100 uses legacy loading scripts blocked at datasets$\geq$3.0; Wikipedia provides equivalent or better content). $^\dagger$Seed 42 uses curriculum warm-start (Stage A: 100 domain-pure steps; Stage B: 400 mixed steps) to resolve router initialisation sensitivity; see Section~\ref{sec:phase2_crosslingual}.}
...
\midrule
42$^\dagger$   & 23.28\% & 45.54\% & 33.69\% & 25.74\% & +22.04\% & GO \\
137  & 23.26\% & 45.52\% & 33.19\% & 25.61\% & +21.79\% & GO \\
2026 & 23.35\% & 45.50\% & 33.20\% & 25.62\% & +21.77\% & GO \\
\midrule
\textbf{Mean (all)} & --- & --- & --- & 25.66\% & +21.87\% & GO ($\pm$0.12pp) \\
\bottomrule
\end{tabular}
\end{table}
