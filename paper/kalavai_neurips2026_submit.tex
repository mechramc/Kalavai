...
inference without any specialist retraining. While prior work shows post-hoc fusion of independently trained models is feasible, this paper characterises when it succeeds, how much gain to expect, and what coordination it requires.
...
Mixture-of-Experts (MoE) routing (500 router training steps on mixed data). Under this protocol, \kalavai improves
...
divergence:
$\text{gain} \approx 0.83 \times \text{divergence} - 2.80$
($R^2 = 0.872$, $n{=}8$ conditions, 3--26\% divergence range). While derived from a limited
number of conditions, this relationship suggests divergence as a practical screening criterion
prior to full cooperative training. Notably, conversion efficiency---the ratio of fusion gain to specialist divergence---increases with model scale ($0.75\times$ at 6.9B vs.\ $0.49\times$ at smaller scales), suggesting the cooperative mechanism strengthens rather than degrades as models become more capable. In high-divergence settings gains are substantially
...
\emph{Routing must be learned}: a trained linear router approaches the domain-level
oracle---a hypothetical router with perfect per-token domain knowledge---with a gap of
${<}10^{-5}$ nats (natural units of entropy; 1~nat $\approx$ 1.44~bits) at 410M scale,
while uniform weight averaging reduces performance by $1.2\%$ relative to the best
individual specialist.
Section~\ref{sec:method} formalises the protocol; Sections~\ref{sec:core}--\ref{sec:divergence_gain}
characterise these conditions empirically; Appendix~\ref{app:evalcorrection} details the evaluation protocol.
\end{abstract}
...
Training competitive language models requires centralised compute at a scale most researchers and institutions cannot access. Frontier training runs demand hundreds of synchronised GPUs for weeks, concentrating capability in organisations with sufficient infrastructure. This creates a practical barrier: strong models are currently accessible primarily to those who can build or fund them. This raises a natural question: can the training effort itself be distributed, so that many contributors---each with their own data, hardware, and constraints---collectively produce a model none of them could build alone? We demonstrate that the answer is yes: language model training can be decentralised into independent specialist contributions, with predictable and quantifiable gains achieved through post-hoc fusion.

\paragraph{Cooperative specialist fusion.} A structurally different approach is possible. If multiple contributors each fine-tune an independent copy of a shared base checkpoint on their own domain, the resulting specialists collectively aggregate complementary knowledge that no single contributor could build alone.

The key insight is that specialists trained on different domains diverge in \emph{complementary} directions from the shared initialisation point: each becomes highly competent on its own domain while degrading on out-of-domain data. A lightweight router---trained on a small mixed-domain dataset after all specialists have submitted their checkpoints---exploits this structure by assigning each input token to its most relevant specialist, recovering specialisation quality across all domains simultaneously. This requires \emph{no communication during training}: contributors work independently, asynchronously, on their own hardware, using their own private data. The only coordination is the shared initial checkpoint.
...
\item \textbf{A predictive divergence--gain relationship} (Section~\ref{sec:divergence_gain}).
Fusion gain is predictable from specialist divergence before training begins:
$\text{gain} \approx 0.83 \times \text{divergence} - 2.80$
($R^2 = 0.872$, $n{=}8$ conditions, 3--26\% divergence range).
...
\item \textbf{Near-oracle routing with a linear router} (Section~\ref{sec:analysis}).
A trained linear router approaches the domain-level oracle (perfect per-token
domain assignment) with a gap of ${<}10^{-5}$ nats at 410M scale. Surprisingly,
routing architecture does not matter---linear and MLP routers perform
equivalently---but routing must be trained; uniform weight averaging degrades
performance below the best individual specialist. The bottleneck is whether the
router is trained at all, not its architectural capacity.

\item \textbf{Consistent gains across model scales} (Section~\ref{sec:core}).
...
\item \textbf{A training-duration crossover for layer freezing} (Section~\ref{sec:crossover}).
...
and provides a measurable performance advantage ($+2.4$pp at 20,000 steps).
Intuitively, short specialisation runs naturally retain compatible lower-level
features; only extended training erodes this shared geometry, at which point
frozen layers act as a structural anchor.

\item \textbf{Substantial gains in high-divergence settings} (Section~\ref{sec:phase2}).
...
\textbf{Design choice: full fine-tuning, not LoRA.} We use full fine-tuning of unfrozen layers rather than parameter-efficient adaptation. LoRA-trained specialists do not develop sufficient representational divergence from the base checkpoint to benefit from fusion: all tested LoRA configurations fall below the empirical divergence floor required for positive gains, confirming that representational divergence---not parameter count---drives cooperative benefit (Appendix~\ref{app:design}, Table~\ref{tab:lora}).
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.70\textwidth,height=0.75\textheight,keepaspectratio]{fig_crosslingual_perplexity}
...
\begin{tabular}{lcccc}
\toprule
Model & Yoruba PPL & Welsh PPL & Tamil PPL & Code PPL \\
\midrule
Base model        & 41.9          & 102.7          & 4.2          & 8.2 \\
\midrule
Yoruba specialist & \textbf{7.7}  & 147.8          & 4.3          & 8.4 \\
Welsh specialist  & 55.4          & \textbf{22.1}  & 4.4          & 8.6 \\
Tamil specialist  & 41.5          & 104.0          & \textbf{3.0} & 8.3 \\
Code specialist   & 47.4          & 107.8          & 4.4          & \textbf{8.1} \\
\midrule
\kalavai MoE      & \textbf{7.7}  & \textbf{22.1}  & \textbf{3.0} & \textbf{8.1} \\
\bottomrule
\end{tabular}
\caption{Cross-domain perplexity matrix (Pythia-410M, seed 137). Bold = own domain / MoE best. Each specialist excels on its own language but degrades severely elsewhere---the Yoruba specialist is \emph{worse than base} on Welsh (147.8 vs.\ 102.7). \kalavai MoE matches every specialist on its own domain simultaneously in a single model.}
\label{tab:crosslingual_matrix}
\end{table}

All three seeds achieve near-identical results ($+21.87\%$ $\pm$0.12pp, mean divergence 25.65\%) with perfect routing ($>$99.98\% gate weight per language). Table~\ref{tab:crosslingual_matrix} shows why single-specialist dispatch fails: deploying the wrong specialist is actively harmful---the Yoruba specialist raises Welsh perplexity from 102.7 to 147.8. \kalavai MoE eliminates this risk. Curriculum warm-start (Stage~A: 100 domain-pure steps; Stage~B: 400 mixed) resolved seed-42 router initialisation sensitivity; all three seeds converge with tight variance.

\subsubsection{20-Contributor Federation (Experiment 3)}
\label{sec:phase2_twenty}

This is the largest cooperative tested: 20 specialists (10 languages + 10 domains) on Pythia-1B. The fused model achieves MoE EW loss \textbf{2.3130} vs.\ best-specialist EW 2.7771---a \textbf{+16.71\%} gain vs.\ best specialist (mean divergence 15.68\%, 3 seeds, $\pm$0.07pp). Two specialists with insufficient data exhibit negative divergence; removing them yields the 18-contributor result: $+21.13\%$ $\pm$0.01pp. 17 of 20 domains achieve $>$98\% correct routing; all 10 language specialists route $>$99.4\% correctly (Appendix~\ref{app:exp3_perdomain}).
...
Across eight experimental conditions spanning 3.16--25.65\% mean divergence (Appendix~\ref{app:divgain_table}), fusion gain scales monotonically with specialist divergence (Figure~\ref{fig:divergence_gain_regression}). Exp~3 (20-contributor, 15.68\%, $+16.71\%$) serves as an out-of-sample validation point.

\paragraph{Linear regression fit.} We fit OLS regression to the eight data points. The linear model ($\text{gain} = -2.80 + 0.83 \times \text{divergence}$) achieves $R^2 = 0.872$ (slope 95\% CI [0.52, 1.15], $n=8$, $t$-distribution). A log-linear fit achieves only $R^2 = 0.683$---the relationship is closer to linear than sublinear across the 3--26\% divergence range. The regression line and 95\% prediction band are shown in Figure~\ref{fig:divergence_gain_regression}.
...
\begin{figure}[h]
  ...
  \caption{Fusion gain vs.\ mean specialist divergence (\%) with OLS regression and 95\% prediction band. Linear fit: $\text{gain} = -2.80 + 0.83 \times \text{div}$ ($R^2 = 0.872$, $n=8$). English-domain conditions cluster near the line; Exp 2 (private, purple) and Exp 1 (cross-lingual, red) lie above it, consistent with outsized gains where the base model is least competent. Exp~3 (20-contributor, 15.68\% div) is an out-of-sample validation point (Appendix~\ref{app:divgain_table}).}
  \label{fig:divergence_gain_regression}
\end{figure}

Below $\approx$3.3\% mean divergence the cooperative is unlikely to yield positive gains; cross-lingual and low-resource settings exceed the English-domain estimate.
...
\paragraph{Router and scaling behaviour.} A trained linear router achieves $+7.70\%$ vs.\ best specialist; a 2-layer MLP achieves $+7.72\%$; untrained uniform routing achieves $-1.19\%$. The 8.9pp gap confirms routing must be learned; the 0.02pp linear-vs-MLP gap confirms architecture is irrelevant once training is adequate (Appendix~\ref{app:router}). Fusion improvement is stable across 410M pre-training maturities ($+7.0\%$--$+8.8\%$) and scales monotonically with specialist count ($+1.76\%$ at 2 to $+12.95\%$ at 5; Appendix~\ref{app:scaling}). At 1B, gain drops to $+0.40\%$ at the fully-trained checkpoint, consistent with reduced specialist divergence from a competent base.

\paragraph{Limitations.} Reduced gain at 6.9B (+6.53\%) is fully explained by lower divergence (8.73\% vs.\ 15.65\%); conversion efficiency is \emph{higher} at 6.9B (0.75$\times$). The $N\times$ inference cost is the price of aggregation: where contributors cannot share data, the comparison is against the absence of any unified model. Sparse top-1 inference loses 21\% quality despite perfect routing (Appendix~\ref{app:inference}). The divergence--gain relationship is derived under a fixed training protocol; real deployment requires contributor vetting and governance.

\paragraph{Future directions.} Routing sparsity emerges without auxiliary loss (2.2 expert switches per prompt; Appendix~\ref{app:routing}). Genuine distributed deployment with asynchronous submission and auxiliary-loss-free load balancing \citep{liu2024deepseekv3} are the primary engineering requirements.

\paragraph{Conclusion.} {\looseness=-1 Independently trained specialists, fused via a lightweight MoE router, consistently outperform the best individual specialist across three tested scales (+7.70\% at 410M, +7.49\% at 1B, +6.53\% at 6.9B). Fusion gain correlates with specialist divergence ($\text{gain} \approx 0.83 \times \text{divergence} - 2.80$, $R^2 = 0.872$, $n{=}8$ conditions). A linear router suffices; shared initialisation is the only coordination requirement. Phase 2 extends to high-divergence settings: private domains achieve $+10.17\%$; cross-lingual fusion achieves $+21.87\%$ (Yoruba PPL $41.9\to7.7$); a 20-contributor federation achieves $+16.71\%$. Gain scales most steeply where the base model is least competent---precisely where decentralised training is most needed. \kalavai enables under-resourced language communities and data-privacy-constrained organisations to collectively train models none could build alone; code and artefacts will be released on publication.}
...
\begin{figure}[htbp]
  ...
  \caption{Expanded divergence--gain scatter ($n=19$). The eight in-sample conditions (circles, including P1/P2 regression extension) are supplemented with eight training-duration crossover conditions (triangles, 50--20{,}000 steps) and three 20-contributor seeds (squares). The linear law ($R^2=0.872$, $n=8$) holds within a fixed training protocol; across training-duration regimes the relationship is non-monotonic (gain rises then falls as specialists over-train). The 20-contributor points (15.7\% divergence, +16.7\% gain) lie above the in-sample regression line, consistent with cooperative size amplifying fusion benefit.}
...
\begin{figure}[t]
  ...
  Cross-lingual conditions (high base PPL) convert divergence most efficiently; English-domain conditions (low base PPL) sit near the baseline conversion rate. Dashed lines are OLS fits; $n=6$ conditions (figure generated on original in-sample set; see main text for updated $n=8$ regression).}
...
\begin{table}[h]
\centering
\caption{Summary of divergence--gain relationship across all Phase 1 and Phase 2 experiments. Predicted gain from linear fit $= -2.80 + 0.83 \times \text{div}$; residual = actual$-$predicted. Predicted values are computed from full-precision OLS coefficients (slope $= 0.8335$, intercept $= -2.7956$). The regression was fit on the eight in-sample conditions (rows 1--8); Exp~3 (row 9) is an out-of-sample validation point. $^{\dagger}$Qwen predicted gain is at the divergence floor ($\approx$3.3\%). $^{\ddagger}$In a 20-specialist cooperative the best specialist is only 0.46\% above base; nominal conversion rate $>1$ reflects this near-equal baseline.}
...
Pythia-6.9B (Ph.1)          & 8.73\%  & +6.53\%  & 0.75$\times$ & +4.48\%  & +2.05pp \\
P1: 2-domain (regression)   & 10.77\% & +6.22\%  & 0.58$\times$ & +6.18\%  & +0.04pp \\
Pythia-1B (Ph.1)            & 15.28\% & +7.49\%  & 0.49$\times$ & +9.94\%  & $-$2.45pp \\
Pythia-410M (Ph.1)          & 15.65\% & +7.72\%  & 0.49$\times$ & +10.25\% & $-$2.53pp \\
Exp 2: private (Ph.2)       & 18.52\% & +10.17\% & 0.55$\times$ & +12.64\% & $-$2.47pp \\
P2: 4-domain (regression)   & 19.84\% & +14.71\% & 0.74$\times$ & +13.74\% & +0.97pp \\
Exp 1: cross-lingual (Ph.2) & 25.65\% & +21.87\% & 0.85$\times$ & +18.58\% & +3.29pp \\
\midrule
\textit{Exp 3: 18-contrib/FE-03 (Ph.2, OOS)} & \textit{19.75\%} & \textit{+21.13\%} & \textit{1.07$\times$$^{\ddagger}$} & \textit{+13.67\%} & \textit{+7.46pp} \\
\bottomrule
\end{tabular}
\end{table}
