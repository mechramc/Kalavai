...
Fusion gains are consistent across scales: $+7.70\%$ at 410M (3-seed mean; seed 42: $+7.72\%$, $\pm$0.02pp), $+7.49\%$ at 1B ($\pm$0.01pp, 3 seeds), $+6.53\%$ at 6.9B ($\pm$0.024pp, 3 seeds).
The fused model achieves oracle-optimal routing: the learned router matches the domain oracle with gap $<10^{-5}$ nats.
Phase 2 extends the protocol to high-divergence settings: cross-lingual fusion (Tamil/Yoruba/Welsh/Code) achieves $+21.87\%$ $\pm$0.12pp (3-seed mean; curriculum warm-start, Section~4.7), with Yoruba perplexity falling $41.9\to7.7$.
...
\begin{table}[h]
\centering
\caption{Phase 2 Experiment 1: Cross-lingual fusion perplexity (Pythia-410M, 3-seed mean with curriculum warm-start---all seeds achieve perfect routing). Code improvement is small because the base model already achieves low perplexity on Python.}
