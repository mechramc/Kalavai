...
Pythia-1B with 20 specialists (10 languages: Tamil, Yoruba, Welsh, Spanish, Hindi, Swahili, Vietnamese, Arabic, Indonesian, Thai; 10 domains: Code, Medical, Legal, Patent, Math, Finance, Chemistry, Fiction, Dialogue, Instructions). Each specialist trains 2,000 steps on its own data; the router trains 1,000 steps (lr\,=\,0.0002, logical batch 20). Result: \textbf{+16.79\%} fusion gain vs.\ best specialist (base EW 2.7898, best-spec EW 2.7771, MoE EW 2.3108; mean divergence 15.71\%, seed 42). STOP/GO verdict: \textbf{GO}.

\paragraph{Router distribution.} The router achieves near-perfect specialisation for 18 of 20 domains (gate weight $\geq$0.984 on own specialist). Two notable patterns: (1) Medical and chemistry share routing (medical: 0.602 medical + 0.384 chemistry; chemistry: 0.527 medical + 0.466 chemistry), consistent with their semantic overlap; this is correct routing behaviour, not confusion. (2) Dialogue ($-$24.9\%) and instructions ($-$16.5\%) degrade vs.\ base, despite near-correct routing (dialogue: 0.971 own gate, instructions: 0.889 own gate). Both were flagged during data loading as having ${<}$500 training chunks (dialogue: 184, instructions: 283); their specialists are undertrained and routing to them causes degradation. Excluding these two data-scarce domains, mean gain across 18 domains is +19.8\%.

\paragraph{Routing saturation.} The 20-specialist result confirms the pattern observed at smaller scales: once shared initialisation is in place, a linear router converges to near-optimal domain assignment. The gate distribution is effectively a lookup table, not a learned function---further evidence that router architecture does not matter.
...
Across all six completed experimental conditions---Qwen-1.5B (mean div.\ 3.16\%), Pythia-6.9B (8.29\%), Pythia-1B (15.28\%), Pythia-410M (15.65\%), Exp 2 private-domain (18.52\%), and Exp 1 cross-lingual (25.65\%)---fusion gain scales monotonically with specialist divergence (Figure~\ref{fig:divergence_gain_regression}).
...
The maturity sweeps at 410M and 1B demonstrate that the \kalavai mechanism is robust across the full training trajectory under the original mixed-batch evaluation protocol. Improvement ranges from approximately 13.4\% to 15.9\% across both scales and all checkpoints. There is no monotonic relationship between pre-training maturity and fusion gain. At 6.9B, the maturity sweep shows lower improvement under original evaluation ($\sim$+2.3--2.4\%), reflecting both the original eval method and reduced specialist training budget; the corrected equal-weight evaluation gives +5.81\% at step10000 (Table~\ref{tab:main}).

All values in Tables~\ref{tab:maturity410}--\ref{tab:maturity6b} use the original mixed-batch evaluation protocol. The corrected equal-weight result at step10000 is +7.72\% at 410M (Table~\ref{tab:main}) and +7.49\% at 1B (Table~\ref{tab:main}); maturity trends are preserved under correction but absolute values differ.
