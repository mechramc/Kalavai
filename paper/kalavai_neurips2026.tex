...
We identify three governing conditions: (1) \emph{Shared initialisation matters for routing stability}: specialists initialised from checkpoints spanning $2\times$ training progress degrade absolute MoE quality by 0.9pp and induce router confusion (code expert receiving 11\% fiction weight); quantitative degradation is modest but routing clarity is substantially reduced. (2) \emph{Frozen layers become necessary beyond $\approx$10,000 training steps}: at short horizons ($\leq$5,000 steps) freezing is optional; beyond $\approx$10,000 steps, it prevents over-specialisation that makes fusion degrade. (3) \emph{Joint inference is required}: single-specialist dispatch with 99.3\%-accurate domain classification degrades performance by $-$21.1\%; all specialists must run in parallel.

Supporting evidence: the fused model beats the best individual specialist by +14.2\% (410M, 3 seeds), +14.8\% (1B), and +2.4\% (6.9B), and outperforms equal-compute monolithic training by +14.5\% (410M) and +14.6\% (1B).

Boundary conditions: Qwen-1.5B at full training shows $-$1.0\% (model-family-specific); 6.9B improvement is reduced due to a ${\sim}34\times$ weaker per-parameter training signal relative to 410M. We do not claim inference efficiency (the fused model is ${\approx}N\times$ more expensive than a single specialist), universal architecture generality, or that downstream benchmarks reliably improve.
\end{abstract}
...
The 6.9B improvement of +2.4\% is smaller than at smaller scales. We attribute this to reduced specialist divergence from base: as Table~\ref{tab:divergence} shows, fusion gain scales directly with the per-domain improvement each specialist achieves over the base model. At 410M and 1B, specialists diverge 10--25\% from base on their domains (mean $\sim$15.5\%), and the fused model improves 14--15\% over the best individual specialist. At 6.9B, specialists diverge only $\sim$2--3\% on their assigned domains---Pythia-6.9B at step 10,000 already handles code, science, and fiction with high competence, leaving little room for specialist improvement at 1,000 training steps. The maturity analysis at 6.9B (step 10,000: +2.43\%; step 143,000: +2.26\%) confirms the effect is stable across training stages, suggesting the limiting factor is specialist divergence rather than pre-training saturation (Appendix~\ref{app:maturity}).

\begin{table}[h]
\centering
\caption{Specialist divergence (per-domain improvement over base, \%) and fusion gain (MoE improvement over best individual specialist, \%), seed 42. Divergence values confirm that fusion gain scales with specialist divergence. $\dagger$~Qwen-1.5B uses code + fiction only (2 specialists; science omitted). 6.9B per-domain breakdown pending full step-sweep analysis (Appendix~\ref{app:inventory}).}
\label{tab:divergence}
\small
\begin{tabular}{lccccc}
\toprule
\textbf{Model} & \textbf{Code div.} & \textbf{Sci.\ div.} & \textbf{Fiction div.} & \textbf{Mean div.} & \textbf{Fusion gain} \\
\midrule
Pythia-410M         & 9.97\%  & 11.60\% & 25.37\% & 15.65\% & +14.17\% \\
Pythia-1B           & 11.05\% & 9.39\%  & 25.41\% & 15.28\% & +14.83\% \\
Pythia-6.9B         & ---     & ---     & ---     & $\sim$2.4\%   & +2.4\%   \\
Qwen-1.5B$^\dagger$ & 1.76\%  & ---     & 4.56\%  & 3.16\%  & $-$0.97\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{\textbf{\kalavai core results.} \textbf{(A)} Fusion improvement over the best individual specialist across model scales: +14.2\% at 410M, +14.8\% at 1B, +2.4\% at 6.9B. \textbf{(B)} Training duration crossover: freeze=0 peaks at 5k steps then degrades; freeze=4 plateaus through 20k steps; crossover at $\approx$10k steps. \textbf{(C)} Routing failure modes: learned MoE routing achieves +14.1\%; single-specialist classifier dispatch degrades by $-$21.1\% (negative values indicate degradation below base). \textbf{(D)} \kalavai vs.\ equal-compute alternatives at 410M: MoE achieves +14.5\% over the equal-compute monolithic baseline. All results are means over 3 seeds.}
  \label{fig:hero}
\end{figure}
...
At 410M, the monolithic baseline achieves +6.7\% over the base model (loss: 2.098), reflecting gradient interference from mixed-domain training \citep{sukhbaatar2024branchtrainmix}. \kalavai with three 2,000-step specialists achieves +14.5\% over this monolithic baseline (Figure~\ref{fig:monolithic}, Table~\ref{tab:monolithic}). The decomposition reveals two separable effects: (1) \emph{Specialisation advantage}: the best individual specialist (+7.1\% vs.\ base) marginally exceeds the monolithic model (+6.7\% vs.\ base), contributing $\approx$0.4pp; (2) \emph{Fusion advantage}: the router then selects the best specialist per token, contributing the remaining $\approx$7.1pp gap over the monolithic model.

At 1B, the monolithic baseline achieves +8.1\% over base (loss: 1.985, std $\pm$0.001, 3 seeds). \kalavai achieves +14.6\% over this monolithic baseline (Table~\ref{tab:monolithic}). The larger monolithic gain at 1B (+8.1\% vs.\ +6.7\% at 410M) does not reduce the cooperative advantage; the \kalavai improvement over monolithic is consistent at both scales ($\approx$14.5--14.6\%).

We note that the monolithic baseline uses a uniform mixture of code, science, and fiction throughout training. A curriculum-based approach (domain-sequential or gradually shifted mixing) could reduce gradient interference and produce a stronger monolithic model, potentially narrowing the advantage. Even if curriculum training closes half the gap, \kalavai still wins by $\sim$7pp on perplexity reduction; moreover, the key structural advantage of zero communication during training---contributors never share data---is independent of the monolithic baseline's strength.

\begin{table}[h]
\centering
\caption{Equal-compute comparison at Pythia-410M and Pythia-1B. Monolithic is trained for 6,000 steps on mixed data. \kalavai uses three specialists trained for 2,000 steps each. All 3 seeds; compute matched. Improvement columns use the base model and monolithic model as baselines respectively.}
\label{tab:monolithic}
\begin{tabular}{llccc}
\toprule
\textbf{Scale} & \textbf{Method} & \textbf{Loss} & \textbf{vs.\ Base} & \textbf{vs.\ Monolithic} \\
\midrule
\multirow{3}{*}{410M}
 & Base model            & 2.248 & ---      & ---       \\
 & Monolithic (6k steps) & 2.098 & +6.7\%  & ---       \\
 & \kalavai MoE          & \textbf{1.793} & +20.2\% & \textbf{+14.5\%} \\
\midrule
\multirow{3}{*}{1B}
 & Base model            & 2.160 & ---      & ---       \\
 & Monolithic (6k steps) & 1.985 & +8.1\%  & ---       \\
 & \kalavai MoE          & \textbf{1.697} & +21.4\% & \textbf{+14.6\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{What the 6.9B result means.} Fusion gain is proportional to specialist divergence---how much a specialist improves over base on its own domain. At 410M and 1B, domain specialists diverge 10--25\% from base (mean $\sim$15.5\%), and fusion gain is 14--15\%. At 6.9B on the same English-text domains, specialists diverge only $\sim$2--3\% from base (mean $\sim$2.4\%), and fusion gain narrows proportionally to +2.4\% (Table~\ref{tab:divergence}). This is not a failure of the protocol: it reflects that Pythia-6.9B at step 10,000 already handles code, science, and fiction with high competence, leaving little room for specialist improvement at 1,000 training steps. \textbf{This predicts that \kalavai will show its largest gains precisely where it is most needed}: low-resource languages, specialised technical domains, and early-stage models where contributors' data fills genuine gaps in the base model's competence. The 6.9B maturity sweep confirms the result is stable (+2.26\% at step 143,000), ruling out pre-training saturation as the cause. A systematic step-budget sweep at 6.9B to recover specialist divergence is reported in Appendix~\ref{app:inventory}.

\paragraph{What the Qwen result means.} Qwen-1.5B represents the saturation endpoint of the divergence mechanism: its code specialist diverges only 1.76\% from base (Table~\ref{tab:divergence}), insufficient for the router to distinguish code from fiction tokens. As a result, the router degrades code performance and the net fusion is marginally negative ($-$0.97\%, $\pm$0.00\%, 3 seeds). Critically, the weight-averaged Qwen ensemble achieves +0.76\% over base while the MoE is $-$0.97\%\,---confirming that the router, not the specialists, is the locus of failure: the code specialist cannot provide a routing signal strong enough for the gate to correctly identify code tokens.

This establishes a practical \emph{routing-signal floor}: when a specialist diverges below $\sim$2\% from base on its domain, the router mislabels tokens and quality regresses. The 6.9B result ($\sim$2.4\% mean divergence) sits above this floor and produces a positive outcome; the Qwen code specialist (1.76\%) is near or below it. The Pythia maturity sweeps (410M and 1B at step 143,000 both show $\sim$14--15\% improvement) confirm this is specific to Qwen's pre-training distribution rather than a universal maturity effect.
...
\begin{itemize}[leftmargin=*, itemsep=1pt]
  ...
  \item We do not claim domain generality beyond English text. All primary experiments use English code, science, and fiction---domains with substantial vocabulary overlap. The 5-domain experiment adds multilingual data (Spanish Wikipedia), but cross-lingual and cross-modal settings remain untested. The application scenarios in this paper (medical, legal, endangered languages) assume domain diversity beyond what our experiments validate.
\end{itemize}
...
Together, these findings validate the core premise of \kalavai: 20 contributors, each training one specialist on their own data with their own hardware, can produce a model that none of them could build alone. The shared initialisation constraint is the primary coordination requirement: our ablation (Section~\ref{sec:sharedinit}) shows that mismatched checkpoints degrade routing clarity, making it the only protocol constraint that contributors must honour.
...
The root cause is insufficient specialist divergence: the Qwen code specialist diverges only 1.76\% from base on code (Table~\ref{tab:divergence}), placing it near or below the practical routing-signal floor. We observe that the weight-averaged Qwen ensemble (+0.76\% over base) outperforms the MoE ($-$0.97\%), confirming that the router---not the specialists---is the locus of failure. With only 1.76\% divergence, the code and fiction specialists produce nearly identical hidden states on code tokens, and the router cannot reliably distinguish them. It routes code tokens to the fiction specialist, degrading code quality and producing a net negative result.

This is qualitatively different from the Qwen model being ``too good'' for cooperative training: the fiction specialist achieves 4.56\% divergence, well above the routing floor. The problem is specifically that Qwen's pre-training corpus includes code in such high-quality and high-volume form that 2,000-step code fine-tuning at this training stage cannot produce enough additional specialisation. The Pythia maturity sweeps (410M and 1B at step 143,000 show $\sim$14--15\% improvement) confirm this is specific to Qwen's pre-training distribution, not a universal maturity effect.
