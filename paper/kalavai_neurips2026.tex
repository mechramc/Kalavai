...
A 20-contributor federation (10 languages + 10 domains) achieves $+16.79\%$ vs.\ best specialist.

Three conditions define the operating envelope: (1)~\emph{Shared initialisation} is required for routing stability; (2)~\emph{Frozen layers} are optional below 5,000 training steps but become necessary beyond that threshold; (3)~\emph{Joint inference} is required---single-specialist dispatch degrades by $-21.1\%$ despite 99.3\%-accurate domain classification.
All results use a corrected per-domain equal-weight evaluation protocol.
\end{abstract}
