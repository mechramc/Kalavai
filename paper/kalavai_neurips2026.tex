...
We identify three governing conditions: (1) \emph{Shared initialisation matters for routing stability}: specialists initialised from checkpoints spanning $2\times$ training progress degrade absolute MoE quality by 0.9pp and induce router confusion; (2) \emph{Frozen layers become necessary beyond $\approx$10,000 training steps}: at short horizons ($\leq$5,000 steps) freezing is optional; beyond $\approx$10,000 steps, it prevents over-specialisation that makes fusion degrade; (3) \emph{Joint inference is required}: single-specialist dispatch with 99.3\%-accurate domain classification degrades performance by $-$21.1\%; all specialists must run in parallel.

Phase 1 results (English domains, corrected equal-weight per-domain evaluation): +7.72\% at 410M (3 seeds, $\pm$0.02\%), +7.49\% at 1B, +5.81\% at 6.9B, +1.06\% at Qwen-1.5B ($\pm$0.01\%, 3 seeds). Fusion gain scales proportionally with specialist divergence at approximately 0.5$\times$ conversion rate on English domains. Routing is near-deterministic ($>$99.9\%) at all tested scales.

Phase 2 extends to high-divergence settings where KALAVAI is most practically valuable: private professional domains (medical/legal/patent) achieve +10.17\% $\pm$0.15pp (3 seeds, mean divergence 18.5\%, conversion 0.55$\times$); cross-lingual fusion (Tamil/Yoruba/Welsh/Code) achieves +21.76\% $\pm$0.005pp on clean seeds (mean divergence 25.65\%, conversion 0.85$\times$), with Yoruba perplexity falling from 41.9 to 7.7 and Welsh from 102.7 to 22.1. % EXP3_PLACEHOLDER: 20-contributor result
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  \item \textbf{Consistent improvement at scale.} Post-hoc MoE fusion beats the best individual specialist by +7.72\% at 410M (3 seeds, $\pm$0.02\%), +7.49\% at 1B, and +5.81\% at 6.9B on equal-weight per-domain evaluation. Gains are proportional to specialist divergence; routing is near-deterministic ($>$99.9\% per-domain gate weight) at all scales tested. Improvement is stable across Pythia training maturities from 3.5\% to 100\% of training.
  ...
  \item \textbf{Specialists must run jointly.} Routing to a single specialist based on a near-perfect (99.3\% accurate) domain classifier \emph{degrades} performance by 21.1\%. Joint inference with all specialists and softmax-weighted combination improves it by +7.72\% vs.\ best specialist (corrected eval) / +16.3\% vs.\ base. Improvement requires specialists to collectively process each token, not classification.
  \item \textbf{Beats equal-compute monolithic training.} A single model trained on mixed data for the same total compute as three specialists achieves +16.0\% over base (corrected eval). Under corrected evaluation, \kalavai MoE beats the monolithic model by +0.47\% at 410M (+0.34\% at 1B), with the main cooperative advantage measured vs.\ the best individual specialist (+7.72\%).
  \item \textbf{Fusion gain scales with divergence.} Across 6 experimental conditions (Qwen to cross-lingual), gain scales at $\approx$0.5$\times$ divergence on English domains and up to 0.85$\times$ on cross-lingual domains. On high-divergence private domains (+18.5\% div): +10.17\% gain. On cross-lingual domains (+25.65\% div): +21.76\% gain.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.82\textwidth]{fig_divergence_gain_scatter}
  \caption{Fusion gain vs.\ best specialist (\%) as a function of mean specialist divergence (\%) across all experimental conditions. English-domain experiments (grey/orange/blue/green) follow an approximate 0.49--0.70$\times$ conversion rate. High-divergence experiments (Exp 2 professional: purple; Exp 1 cross-lingual: red) show higher conversion rates (0.55$\times$ and 0.85$\times$ respectively). The conversion rate annotation (number beside each point) is gain/divergence. Dashed line: linear fit to English-domain data points.}
  \label{fig:divergence_gain}
\end{figure}

\begin{table}[h]
\centering
\caption{Summary of divergence--gain relationship across all Phase 1 and Phase 2 experiments. Conversion rate = fusion gain / mean divergence. Higher-divergence settings convert more efficiently; the lowest conversion rate (0.34$\times$) occurs at the lowest divergence (Qwen, 3.16\%) where the base model already covers both specialist domains well.}
\label{tab:divergence_gain}
\small
\begin{tabular}{lcccc}
\toprule
\textbf{Condition} & \textbf{Domains} & \textbf{Mean Div.} & \textbf{Gain vs Spec} & \textbf{Conv.\ rate} \\
\midrule
Qwen-1.5B (Ph.1)         & code, fiction              & 3.16\%  & +1.06\%  & 0.34$\times$ \\
Pythia-6.9B (Ph.1)       & code, science, fiction     & 8.29\%  & +5.81\%  & 0.70$\times$ \\
Pythia-1B (Ph.1)         & code, science, fiction     & 15.28\% & +7.49\%  & 0.49$\times$ \\
Pythia-410M (Ph.1)       & code, science, fiction     & 15.65\% & +7.72\%  & 0.49$\times$ \\
Exp 2: private (Ph.2)    & medical, legal, patent     & 18.52\% & +10.17\% & 0.55$\times$ \\
Exp 1: cross-lingual (Ph.2) & Tamil, Yoruba, Welsh, code & 25.65\% & +21.76\% & 0.85$\times$ \\
\bottomrule
\end{tabular}
\end{table}

The divergence-gain relationship has a practical implication for deploying KALAVAI: the protocol is most valuable precisely where contributors can contribute the most unique data. Low-resource languages (Yoruba diverges 45.5\%), specialised professional domains (legal diverges 34.2\%), and any setting where contributors' data fills genuine gaps in the base model's training distribution will maximise specialist divergence and thus fusion gain. English general-domain contributors, by contrast, produce specialists that diverge less (code 10\%, science 7--12\%) and yield proportionally smaller gains.
