...
\title{KALAVAI: Cooperative LLM Training via Independent Specialists\\
       Outperforms Equal-Compute Monolithic Training}

\author{%
  Ramchand Kumaresan \\
  Murai Labs \\
  \texttt{ram@murailabs.com}
}
...
\begin{abstract}
We present \kalavai\footnote{KALAVAI (\textit{kalaVAI}) is the ISO~15919 romanisation of the Tamil word for ``fusion'' or ``mixing''.}, a cooperative protocol for training large language models without centralised compute. In \kalavai, multiple independent contributors each fine-tune a domain-specialist copy of a shared base checkpoint on their own data, then combine their models post-hoc via a lightweight mixture-of-experts (MoE) router. The fused model outperforms any individual specialist on held-out mixed-domain evaluation.

We validate \kalavai across three scales (Pythia-410M, 1B, and 6.9B) using three independently trained domain specialists (code, science, fiction). The fused model surpasses the best individual specialist by \textbf{+14.2\%} (410M, 3 seeds, $\pm$0.016\%), \textbf{+14.8\%} (1B, 3 seeds, $\pm$0.003\%), and \textbf{+2.4\%} (6.9B, 3 seeds, $\pm$0.00\%) on held-out perplexity. Against an equal-compute monolithic baseline trained for $3\times$ as many steps on mixed data, the fused model achieves a \textbf{+14.5\%} advantage.

Three empirical findings distinguish our work from prior branch-train-mix methods: (1) a training duration crossover showing that frozen layers are optional at short training horizons but become necessary beyond approximately 10,000 specialist training steps; (2) a capacity control ruling out parameter count as the explanation for the improvement; and (3) a counterintuitive routing failure showing that single-specialist dispatch with 99.3\%-accurate domain classification degrades performance by 21.1\%, while running all specialists improves it by 14.2\%. The mechanism is cooperative specialisation combined with joint inference, not routing sophistication.
\end{abstract}
...
\paragraph{Branch-Train-Mix (BTX).} The closest prior work is BTX \citep{sukhbaatar2024branchtrainmix}, which demonstrates that models branched from a shared checkpoint, independently trained, and mixed via MoE routing form a better model than any individual branch. Our work extends BTX in three directions: we provide the first empirical training duration crossover showing when frozen layers transition from optional to necessary; we compare against an equal-compute monolithic baseline; and we demonstrate catastrophic failure of single-specialist dispatch, which BTX does not analyse.
...
\]
where $h_t$ is the hidden state at layer $K+1$ (first non-frozen layer), $N$ is the number of specialists, and $d$ is the hidden dimension. At inference, all $N$ specialists process each token in parallel and the output is a weighted combination of their logit distributions:
\[
...
A natural simplification of the \kalavai architecture would route each input to a single specialist rather than combining all specialists' outputs. If the router achieves high classification accuracy, this would reduce inference cost by $N\times$. We test this hypothesis directly.

We train a logistic regression classifier on mean-pooled frozen backbone representations to predict the domain of each input. The classifier achieves 99.3\% accuracy on held-out inputs. We then use this classifier to route each token sequence to the single matching specialist at inference time, suppressing the other two specialists entirely.

The result is catastrophic: single-specialist dispatch with 99.3\%-accurate routing achieves a mixed-domain loss of 2.931, representing a \emph{$-$21.1\% degradation} relative to the base model (loss 2.420) rather than the +14.1\% improvement from joint inference (Table~\ref{tab:dispatch}). An alternative multi-head baseline---three independent output heads on a shared frozen trunk, each trained on one domain, with classifier-based selection---achieves the identical $-$21.1\% degradation, ruling out the single-model-with-heads variant.
...
The mechanism is catastrophic forgetting. Each specialist is fine-tuned on a single domain and, in the process, loses accuracy on the other two domains. The code specialist evaluated on science text (loss 2.908) is \emph{worse than base} (loss 2.892). The science specialist evaluated on fiction text (loss 3.061) is substantially worse than base (loss 2.974). When only one specialist runs and the input happens to cross domain boundaries, there is no fallback. Joint inference with all specialists restores coverage for every token.

This finding has an important implication: the improvement from \kalavai is not primarily about routing quality. Hard routing (argmax, all experts run) achieves +14.1\%---essentially identical to soft routing's +14.2\%. Whether the combination is a weighted average or a selection among all specialists is immaterial; what matters is that all specialists participate in every forward pass.
...
\paragraph{Improvement is robust across training maturities.} We evaluate fusion at six Pythia checkpoints (step 5,000 through step 143,000) at 410M and five checkpoints at 1B. At 410M, improvement ranges from +13.4\% to +15.0\%; at 1B, from +13.8\% to +15.9\%. The mechanism does not depend on the base model being under-trained. Full maturity curves in Appendix~\ref{app:maturity}.

\paragraph{Improvement is robust across specialist count.} Scaling from 2 to 5 specialists on five domains: 2 specialists achieve +17.7\% (narrower evaluation makes the score higher), 3--5 specialists all achieve approximately +14.1\% with near-zero variance. The gain neither grows nor declines substantially as more specialists are added. Full scaling analysis in Appendix~\ref{app:scaling}.

\paragraph{Token-level routing confirms mid-sequence switching.} On five hybrid-domain prompts (e.g., ``Derive the equation for protein folding using Python pandas''), the router produces 11 expert switches across the full prompt sequences (2.2 switches per prompt on average). On the prompt above, the router assigns fiction-domain weights to narrative tokens (``Derive'', ``the'', ``equation''), science-domain weights to biology tokens (``protein'', ``folding''), and code-domain weights to programming tokens (``Python'', ``pandas''). This confirms the router operates at token granularity, not at document level. Routing visualisations in Appendix~\ref{app:routing}.
...
\paragraph{Architecture and scale.} All Pythia experiments use the GPT-NeoX architecture. The Qwen result provides a second architecture data point. We do not have results for Llama, Gemma, or Mistral architectures; cross-architecture validation is an important direction for future work.

\paragraph{No real cooperative has been run.} All experiments use a simulated cooperative where specialists are trained sequentially on the same machine. The protocol assumes contributors can verify checkpoint hashes and adhere to architecture specifications---reasonable for small groups but potentially challenging at scale. Privacy-preserving cooperative variants (e.g., ZKLoRA \citep{zklora2025} for verifiable training provenance) could address the trust problem.

\paragraph{Primary metric is held-out perplexity.} Downstream task accuracy improvements at the 1B scale are small ($<$1pp on most benchmarks). Perplexity is the primary signal because the specialists are trained on domain-specific language modelling tasks, not downstream tasks. Whether perplexity gains translate to task gains at 6.9B and beyond requires larger-scale evaluation.

\paragraph{Applications.} The zero-communication-during-training property of \kalavai enables cooperative training scenarios that are infeasible with synchronous methods: multi-hospital medical language models where patient data cannot leave the facility; multi-jurisdictional legal AI where training data is subject to national regulations; sovereign AI for small nations where no single entity can afford frontier training but a consortium could; low-resource language coverage where each language community trains a specialist on their language and all are fused.
...
\begin{figure}[h]
  ...
  \caption{Mixed-domain held-out loss for each method at Pythia-410M scale. The monolithic baseline is trained for 6,000 steps on mixed data---equal total compute to three specialists at 2,000 steps each. \kalavai MoE (1.793) improves over both the best specialist (2.089) and the monolithic baseline (2.098). Weight averaging (2.158) underperforms the best specialist. Error bars are smaller than markers (variance across 3 seeds: $\pm$0.016\%).}
