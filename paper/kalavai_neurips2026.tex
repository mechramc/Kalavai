...
Supporting evidence: the fused model beats the best individual specialist by +7.72\% (410M, 3 seeds, $\pm$0.02\%), +7.49\% (1B), and +5.81\% (6.9B) on equal-weight per-domain evaluation (equal batch size across all models). Gains are proportional to specialist divergence: 410M and 1B specialists diverge $\sim$15.5\% from base and achieve $\sim$7.5\% fusion gain; 6.9B specialists diverge only 8.29\% and achieve +5.81\%. Routing is near-deterministic ($>$99.9\% per-domain gate weight) at all scales tested. Qwen-1.5B achieves +1.06\% ($\pm$0.01\%, 3 seeds) with perfectly deterministic routing.

We do not claim inference efficiency (the fused model is ${\approx}N\times$ more expensive than a single specialist), universal architecture generality, or that downstream benchmarks reliably improve.
\end{abstract}
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  ...
  \item \textbf{Beats equal-compute monolithic training.} A single model trained on mixed data for the same total compute as three specialists achieves +6.7\% over base (old eval) / +16.0\% over base (corrected equal-weight eval). Under corrected evaluation, \kalavai and the monolithic model achieve comparable equal-weight loss, with the cooperative advantage primarily measured vs.\ the best individual specialist rather than vs.\ monolithic.
\end{enumerate}
