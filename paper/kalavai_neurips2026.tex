...
\paragraph{Scale validation.} The +16.79\% gain at 15.71\% mean divergence is +6.68pp above the linear regression prediction (+10.11\%) from Section~\ref{sec:divergence_gain}, consistent with the pattern that heterogeneous multi-domain cooperatives---mixing language and domain specialists with widely varying divergence levels---outperform the English-domain regression baseline. The routing saturation result holds at scale: a simple linear router on 20 specialists converges to near-optimal domain assignment, confirming that router architecture does not matter once shared initialisation is in place.
