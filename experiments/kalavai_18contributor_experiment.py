...
"""
KALAVAI Phase 2, Experiment 3: 18-Contributor Cooperative
==========================================================
10 languages (cc100) + 8 domains (public HF datasets).
Base: Pythia-1B step10000 (hidden_size=2048).

Design:
  - 18 specialists trained independently (2000 steps each, FREEZE_LAYERS=0)
  - MoE fusion via linear router: nn.Linear(2048, 18) — one per expert
...
RESULTS_DIR    = Path(os.environ.get("KALAVAI_RESULTS_DIR",    "results/phase2/eighteen_contributor"))
...
SPECIALISTS = LANGUAGE_SPECIALISTS + DOMAIN_SPECIALISTS   # length 18
...
# ============================================================================
# Architecture — 18-Expert MoE with linear router
#
...
class TwentyExpertMoE(nn.Module):
    ...
    def forward(self, input_ids, labels=None):
        ...
        all_logits, all_h = [], []
        gpu_mode = self._gpu_models is not None
        ...
        if gpu_mode:
            ...
            for m in self._gpu_models:
                ...
                all_logits.append(out.logits.float())
                all_h.append(out.hidden_states[-1].float().mean(dim=1))
        else:
        ...
        h_mean = torch.stack(all_h, dim=0).mean(dim=0)
        if not gpu_mode:
            h_mean = h_mean.to(self.device)
        ...
        for i, logit in enumerate(all_logits):
            ...
            weighted = gates[:, i, None, None] * logit                    # (B, T, V)
...
def main():
    ...
    for seed in SEEDS:
        if args.router_only:
            ...
            print(f"\n  git commit: [kalavai] phase2 exp3 18contributor seed={seed} router-retry: {imp:+.2f}% vs spec")
            continue
        ...
        imp = result["metrics"]["improvement_vs_spec"]
        print(f"\n  git commit: [kalavai] phase2 exp3 18contributor seed={seed}: {imp:+.2f}% vs spec")

    print("\n── Phase 2 Experiment 3 (18-contributor) complete ─────────────────────")
